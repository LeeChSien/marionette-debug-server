...
LogReporter.prototype = {
  ...
  exprotImage: function(data, log_path, filename, memoryPool) {
    ...
    for(var i = 0; i < data.screenshots.length; i++) {
      ...
      var entity = memoryPool.findByDescription(data.screenshots[i].description);
      if(entity) {
  ...
  savePDFReport: function(path, log_path, filename, memoryPool, part) {
    ...
    for(var i = 0; i < results.length; i++) {
      ...
      doc.image(log_path + '/' + results[i].pic[part], 50, 70, { fit: [300, 300] })
      doc.text('');
    }
  ...
  savePDFDiff: function(path, log_path, filename, memoryPool) {
    ...
    for(var i = 0; i < results.length; i++) {
      doc.addPage();
      doc.text(results[i].description, 50, 50);
      doc.image(log_path + '/' + results[i].pic.first, 50, 70, { fit: [300, 300] });
      doc.image(log_path + '/' + results[i].pic.second, 350, 70, { fit: [300, 300] });
      doc.image(log_path + '/' + results[i].pic.diff, 50, 400, { fit: [300, 300] });
      doc.text('');
    }
